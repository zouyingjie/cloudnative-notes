import{_ as p,r as i,c as o,o as c,d as l,a as s,b as a,e as t}from"./app-ouoXKg5d.js";const u={},r={href:"https://kubernetes.github.io/ingress-nginx/user-guide/nginx-configuration/annotations/#canary",target:"_blank",rel:"noopener noreferrer"},d={href:"https://github.com/megaease/easemesh",target:"_blank",rel:"noopener noreferrer"};function m(k,n){const e=i("ExternalLinkIcon");return c(),o("div",null,[n[4]||(n[4]=l(`<h1 id="_3-控制器对象" tabindex="-1"><a class="header-anchor" href="#_3-控制器对象" aria-hidden="true">#</a> 3.控制器对象</h1><p>在使用 Kubernetes 时，我们通常是使用各种 Controller 对象来管理 Pod。 每个 Controller 本质上就是观察对象状态并进行状态拟合的控制循环。每个 Controller 都是一个单独的进程，试图将对象的状态调整为其所期望的状态。</p><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code><span class="token keyword">for</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>ReplicaSet 的名称满足 <code>[Deployment-Name]-[Random-String]</code> 格式， 其中 <code>RANDOM-STRING</code> 是随机生成的，并以 pod-template-hash 为种子。</p><p>pod-Template-hash 是由 Deployment Controller 添加在其创建或接管的 ReplicaSet 上的标签，该标签会保证一个 Deployment 下的子 ReplicaSet 不会重复。该标签由 ReplicaSet 下的 PodTemplate 通过 hash 处理生成的，并将结果作为 Pod 的标签，并添加到 ReplicaSet 的选择器上，注意不要更改这个标签。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ kubectl describe rs nginx-deployment-559d658b74
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>必须在 Deployment 中指定合适的 selector 和 Pod 标签，不要和其他 Controller 的 Label 或 selector 重复。<strong>Kubernetes 不会检查并阻止 Controller 之间的 label/selector 冲突，如果多个 Controller 有相同的 lable/selector， 可能会导致 Pod 管理上的冲突，引发预期之外的行为。</strong></p><h4 id="_2-2-水平伸缩与升级" tabindex="-1"><a class="header-anchor" href="#_2-2-水平伸缩与升级" aria-hidden="true">#</a> 2.2 水平伸缩与升级</h4><h5 id="_1-水平伸缩" tabindex="-1"><a class="header-anchor" href="#_1-水平伸缩" aria-hidden="true">#</a> 1) 水平伸缩</h5><p>普通的伸缩命令</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>kubectl scale deployment.v1.apps/nginx-deployment <span class="token parameter variable">--replicas</span><span class="token operator">=</span><span class="token number">10</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h5 id="_2-更新镜像" tabindex="-1"><a class="header-anchor" href="#_2-更新镜像" aria-hidden="true">#</a> 2) 更新镜像</h5><p>使用如下命令</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>kubectl <span class="token builtin class-name">set</span> image deployment/nginx-deployment <span class="token assign-left variable">nginx</span><span class="token operator">=</span>nginx:1.16.1 <span class="token parameter variable">--record</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>除此之外还可以通过 <code>kubectl edit deployments.apps nginx-deployment</code> 或者 <code>kubectl apply -f nginx-deployment.yml</code> 来更新 PodTemplate。</p><h5 id="_3-升级策略" tabindex="-1"><a class="header-anchor" href="#_3-升级策略" aria-hidden="true">#</a> 3) 升级策略</h5><p>Deployment 升级有两种策略：</p><ul><li><strong>ReCreate</strong> ：重建，先将旧的 Pod 全部干掉，然后在创建新的。</li><li><strong>Rolling Update</strong>：滚动更新，默认策略</li></ul><p>滚动更新时，Deployment 允许在同一个时刻存在多个不同版本实例。当滚动更新 Deployment 时，在 Deployment 展开过程中（正在展开或暂停），DeploymentController 会平衡新增的副本和当前活跃的副本以降低风险。 这被称为按比例扩展。</p><p>Deployment 主要通过 <strong>maxSurge</strong> 和 <strong>maxUnavailable</strong> 两个字段控制新旧版本的比例：</p><ul><li><strong>maxSurge</strong>：允许的最多超出的 Pod 数量。默认值为 25%，计算方式是四舍五入，也可以设置绝对值。比如 Pod 数为 4，那么默认是 4 * 25% = 1，即最多允许超出 1 个，也就是说在滚动升级期间，最多可以有 5 个 Pod 运行。</li><li><strong>maxUnavailable</strong>：最多允许多少 Pod 不可用。默认值为 25，计算方式是四舍五入，也可以设置绝对值。比如 Pod 数为 4，那么默认是 4 * 25% = 1，即最多允许 1 个 Pod 不可用，也就是说在滚动升级期间，最少有 3 个 Pod 运行。</li></ul><h5 id="_4-rollover" tabindex="-1"><a class="header-anchor" href="#_4-rollover" aria-hidden="true">#</a> 4) Rollover</h5><p>当 Deployment 更新时，其会创建新的 ReplicaSet 并将 Pod 部署为期望的数量。如果在展开的过程中又有更新，此时 Deployment 会再次创建新的副本，并终止扩展之前的副本。</p><p>例如，我们在创建一个副本为 5 镜像是 nginx:1.14.2 的 Deployment， 目前只有 3 个副本存在，此时更新 Deployment 的镜像为 nginx:1.16.1 。在此场景下，Deployment 会立刻杀掉已经创建的 3 个 nginx:1.14.2 的副本， 并开始创建 nginx:1.16.1 Pod 的副本，它并不会等 5 个 nginx:1.14.2 都创建完才开始 nginx:1.16.1 副本的更新。</p><h6 id="_1-回滚" tabindex="-1"><a class="header-anchor" href="#_1-回滚" aria-hidden="true">#</a> 1) 回滚</h6><p>查看历史版本</p><p>当更新 Deployment 的 PodTemplate 进行升级时，Kubernetes 会保存升级历史，可以通过 kubectl rollout history 命令查看。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>kubectl rollout <span class="token function">history</span> deployment.v1.apps/nginx-deployment
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-3-canary-deployment" tabindex="-1"><a class="header-anchor" href="#_2-3-canary-deployment" aria-hidden="true">#</a> 2.3 Canary Deployment</h4><p>利用 Deployment 可以实现一个粗糙的灰度部署，可以部署两个版本的 Deployment 并设置不同的副本。这样流量可以按比例流入不同的版本中。</p><div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> apps/v1
<span class="token key atrule">kind</span><span class="token punctuation">:</span> Deployment
<span class="token key atrule">metadata</span><span class="token punctuation">:</span>
...
<span class="token key atrule">spec</span><span class="token punctuation">:</span>
 ...
 <span class="token key atrule">template</span><span class="token punctuation">:</span>
   ...
   <span class="token key atrule">spec</span><span class="token punctuation">:</span>
     ...
     <span class="token punctuation">-</span> <span class="token key atrule">name</span><span class="token punctuation">:</span> my<span class="token punctuation">-</span>app
       <span class="token key atrule">image</span><span class="token punctuation">:</span> containersol/k8s<span class="token punctuation">-</span>deployment<span class="token punctuation">-</span>strategies
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>虽然使用 Deplpyment 可以实现一个基本的灰度部署，但在实际场景中其功能还是太弱了，无法对灰度流量做更多的控制。实际应用中，我们往往需要对灰度流量做更多的控制，比如基于请求的 header、cookie 等进行灰度，此时就需要使用其他方案。下面是一些常见的灰度方案：</p>`,87)),s("ul",null,[s("li",null,[s("a",r,[n[0]||(n[0]=a("Ingress-Nginx#Canary")),t(e)])]),s("li",null,[n[2]||(n[2]=a("Istio、")),s("a",d,[n[1]||(n[1]=a("EaseMesh")),t(e)]),n[3]||(n[3]=a(" 等 ServiceMesh 方案"))])]),n[5]||(n[5]=l(`<h4 id="_2-4-autoscaling-hpa" tabindex="-1"><a class="header-anchor" href="#_2-4-autoscaling-hpa" aria-hidden="true">#</a> 2.4 AutoScaling &amp; HPA</h4><p>Deployment 可以让我们手动的进行 Pod 的水平扩缩容操作，但在遇到流量压力时，无论是应用层面的 Pod，还是基础设施层面的 Node，人工操作都不容易做到及时和准确的响应，因此 Kubernetes 提供了 AutoScaler 来进行快速的自动伸缩。</p><p>Kubernetes 中的自动伸缩可以分为三类：</p><ul><li><p>Pod 水平扩缩容（HPA）</p></li><li><p>Pod 垂直扩缩容（VPA）</p></li><li><p>集群扩缩容 这里主要看下水平伸缩。HPA 目前支持下面四种资源的自动水平伸缩</p></li><li><p>Deployment</p></li><li><p>StatefulSet</p></li><li><p>ReplicaSet</p></li><li><p>ReplicaController</p></li></ul><p>下面是官网中的一个示例，首先创建一个 Deployment 作为水平伸缩的对象</p><div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> apps/v1
<span class="token key atrule">kind</span><span class="token punctuation">:</span> Deployment
<span class="token key atrule">metadata</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>也可以通过 yaml 创建：</p><div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">​​apiVersion</span><span class="token punctuation">:</span> autoscaling/v2beta2
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_3-job-cronjob" tabindex="-1"><a class="header-anchor" href="#_3-job-cronjob" aria-hidden="true">#</a> 3. Job/CronJob</h3><p>像 Deployment 这种是对在线业务的抽象，在实际场景中我们有很多离线任务，比如定时脚本、离线计算等。Kubernetes 使用 Job 和 CronJob 来执行此类任务。</p><h4 id="_3-1-job" tabindex="-1"><a class="header-anchor" href="#_3-1-job" aria-hidden="true">#</a> 3.1 Job</h4><p>Job 是对离线任务的抽象，可以用来执行离线计算、批处理任务等。其使用定义如下：</p><div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> batch/v1
...
<span class="token comment"># │ │ │ │ │</span>
<span class="token comment"># *  *  *  *  *</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>使用 CronJob 时需要注意：</p><ul><li>CronJob 并不能保证准时执行，可能会有一定的滞后。</li><li><strong>concurrencyPolicy</strong>: 并发策略。因为是定时任务，存在当前任务要执行时，上一个任务还没有结束的情况。该字段用来设置其处理策略。 <ul><li>Allow: 默认值，允许任务同时存在。</li><li>Forbid: 该周期内的任务被跳过，Job 会被标记为 miss。</li><li>Replace: 新的 Job 会替换旧的未完成的 Job。</li></ul></li><li>如果在 CronJob 在一段时间内 miss 数据达到 100，则该 CronJob 会被停止。该时长通过 startingDeadlineSeconds 字段设置，但不要小于 10s。</li></ul><p>和其他控制器运行方式不同，CronJobController 并不是使用 watch 监听机制从 Informer 接收变更信息的，而是每隔 10s 主动访问 api-server 获取数据。因为其每隔 10s 访问 api-server 检查资源，因此如果 startingDeadlineSeconds 设置小于 10s，可能会导致 CronJob 无法被调度。</p><h3 id="_4-statefulset" tabindex="-1"><a class="header-anchor" href="#_4-statefulset" aria-hidden="true">#</a> 4, StatefulSet</h3><p>Deployment 所代表的是无状态服务，由其管理的 Pod 的可以随意的进行上下线、扩缩容，新加的 Pod 可以部署在任意符合条件的节点上。但在实际情况中除了无状态的、可以随意部署灵活伸缩的应用，在分布式系统中，我们还有很多有状态的应用，每个实例有自己的存储数据，实例之间也有主从等特定的关系，如果我们将这样一个 Pod 随意下线上线，那应用重启后就会丢失数据，导致应用执行失败，因此这类应用需要更加特殊的处理，保证其重新部署后数据不会丢失，节点之间的关系不会发生变化。</p><p>Kubernetes 提供了 StatefulSet 来管理有状态应用的资源对象。StatefulSet 早期也叫 PetSet。应用服务可以分为两类，一种是宠物模式，一种是奶牛模式：</p><ul><li>宠物模式：宠物有自己单独的名字，我们必须悉心照料每一只宠物，宠物生病时一定要治好救活，主要是对应有状态的服务。</li><li>奶牛模式：有些应用像是奶牛，如果奶牛出了问题我们新换一只就行，不用费心太多，主要是对应无状态的服务。</li></ul><p>Deployment 管理的无状态应用就像是奶牛，而有状态应用就是在宕机后必须恢复原样的那只“宠物”。应用中的状态可以分为两类：</p><ul><li><strong>拓扑状态</strong>：应用中的多个实例并不是完全对等的，它们之前有特殊的关系，这意味着实例的启动需要按照一定的顺序进行。比如 MySQL 的主从，必须先启动主节点，在启动从节点，而终止顺序则应该相反。除此之外，新启动的 Pod 应该和原来网络标识是一样的，以便用户可以使用相同的方式访问。</li><li><strong>存储状态</strong>：多个实例的数据存储是固定的并且数据不能丢失。Pod A 的数据，在 Pod A 重启后依然可以被访问到。</li></ul><p>因此由 StatefulSet 管理的 Pod 有下面一些特性：</p><ul><li><p>Pod 会按顺序创建和销毁：创建后续 Pod 前，必须保证前面的 Pod 已经这个在正常运行进入 Ready 状态。删除时会按照与创建相反的顺序删除，并且只有在后面的 Pod 都删除成功后才会删除当前 Pod。</p></li><li><p>StatefulSet 的 Pod 具有稳定且唯一的标识，包括顺序标识、稳定的网络标识和稳定的存储。该标识和 Pod 是绑定的，不管它被调度在哪个节点上。对于具有 N 个副本的 StatefulSet，StatefulSet 中的每个 Pod 将被分配一个整数序号，从 0 到 N-1，该序号在 StatefulSet 上是唯一的。</p></li><li><p>StatefulSet 的 Pod 具有稳定的持久化存储，每个 Pod 都可以拥有自己的独立的 PVC 资源，即使Pod被重新调度到其他节点上，它所拥有的持久化存储也依然会被挂载到该Pod。</p></li><li><p>StatefulSet 中的每个 Pod 根据 StatefulSet 的名称和 Pod 的序号派生出它的主机名。 组合主机名的格式为 <code>$(StatefulSet name)-$(序号)</code>。 比如，一个叫 web 的 StatefulSet 的三个 Pod 名称分别为 web-0、web-1、web-2 。</p></li></ul><p>通过固定且唯一的标识，不变的持久化存储，Kubernetes 可以保证在 Pod 重建后依然保留上次的运行状态，从而运行有状态和应用。</p><p>下面是 StatefulSet 的一个示例：</p><div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> apps/v1
...
<span class="token key atrule">spec</span><span class="token punctuation">:</span>
 ...
 <span class="token key atrule">selector</span><span class="token punctuation">:</span>
   <span class="token key atrule">app</span><span class="token punctuation">:</span> nginx
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>和默认 Service 不同，Headless 没有自己的 IP 地址，经由 Headless 代理的 Pod，每个 Pod 创建时都会创建独立的 DNS 记录，格式为 <code>$(pod-ip).$(svc-name).namespace.svc.cluster.local</code> ，如： <code>web-1.nginx.default.svc.cluster.local</code>。虽然 Pod 会发生重建，其 IP 也会发生变化，但我们通过上面的 DNS 域名就可以实现对 Pod 的固定访问。</p><p>同时对于 Pod 之间的通信，Headless 类型的服务也会创建 <code>svc-name.namespace.svc.cluster.local</code> 域名的 DNS 记录，但是返回的是所有 Pod 的 IP。</p><h4 id="_4-2-数据存储" tabindex="-1"><a class="header-anchor" href="#_4-2-数据存储" aria-hidden="true">#</a> 4.2 数据存储</h4><p>为了实现数据存储，StatefulSet 需要创建持久卷声明，一个 StatefulSet 可以拥有一个或多个持久卷声明模板，在 Pod 创建之前会先创建这些 PVC 并作为 volume 绑定到 Pod 上。</p><p>因为默认声明了 PVC 因此必须要有 PV 来提供存储，给定 Pod 的存储可以由 PVC 模板指定的 StorageClass 自动创建，也可以由集群管理员预先提供。</p><p>另外在删除或者伸缩 StatefulSet 时并不会自动删除它关联的存储卷，必须要要手动删除，这样做主要是为了保证数据安全，避免数据误删。</p><p>下面是官方文档提到的使用 StatefulSet 时的一些限制和注意事项：</p><ul><li><p>删除 StatefulSets 时，StatefulSet 不提供任何终止 Pod 的保证。为了实现 StatefulSet 中的 Pod 可以有序和优雅的终止，可在删除之前将 StatefulSet 缩放为 0。</p></li><li><p>在默认 Pod 管理策略(OrderedReady) 时使用 Rolling Update，可能进入需要 人工干预 才能修复的损坏状态。</p></li><li><p>必须设置 StatefulSet 的 .spec.selector 字段，使之匹配其在 .spec.template.metadata.labels 中设置的标签。在 Kubernetes 1.8 版本之前，被忽略 .spec.selector 字段会获得默认设置值。在 1.8 和以后的版本中，未指定匹配的 Pod 选择器将在创建 StatefulSet 期间导致验证错误。</p></li><li><p>当 StatefulSet 创建 Pod 时，它会添加一个标签 statefulset.kubernetes.io/pod-name，该标签设置为 Pod 名称。通过该标签可以给 StatefulSet 中的特定 Pod 绑定一个 Service。</p></li><li><p>在 Kubernetes 1.7 及以后的版本中，StatefulSet 的 .spec.updateStrategy .spec.updateStrategy 字段让您可以配置和禁用掉自动滚动更新 Pod 的容器、标签、资源请求或限制、以及注解。</p></li></ul><h3 id="_5-daemonset" tabindex="-1"><a class="header-anchor" href="#_5-daemonset" aria-hidden="true">#</a> 5. DaemonSet</h3><p>DeamonSet 确保全部（或者某些）节点上运行一个 Pod 的副本。 当有节点加入集群时， 也会为他们新增一个 Pod 。 当有节点从集群移除时，这些 Pod 也会被回收。删除 DaemonSet 将会删除它创建的所有 Pod。一般用于“监控进程”、“日志收集”或“节点管控”。</p><div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> v1
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_6-总结" tabindex="-1"><a class="header-anchor" href="#_6-总结" aria-hidden="true">#</a> 6. 总结</h3><p>可以看到 Kubernetes 的控制器基本覆盖了日常使用的大部分场景，通过这些控制器我们可以将应用部署到集群中，并进行灵活的扩缩容、滚动更新等操作。原先需要在操作系统层面进行的部署、配置工作，现在都可以通过创建、配置不同的 Kubernetes Controller 对象来完成，从这一点来看，说 Kubernetes 是云计算时代的操作系统并不为过。</p>`,62))])}const b=p(u,[["render",m],["__file","03.controller.html.vue"]]);export{b as default};
