import{_ as t,r as p,c as o,o as c,a as s,d as l,b as a,e as i}from"./app-ouoXKg5d.js";const r={},u={href:"https://kustomize.io/",target:"_blank",rel:"noopener noreferrer"},d={href:"https://helm.sh/",target:"_blank",rel:"noopener noreferrer"},v={href:"https://helm.sh/docs/",target:"_blank",rel:"noopener noreferrer"},m={href:"https://learning.oreilly.com/library/view/learning-helm/9781492083641/",target:"_blank",rel:"noopener noreferrer"},k={href:"https://chartmuseum.com/",target:"_blank",rel:"noopener noreferrer"},b={href:"https://github.com/operator-framework/awesome-operators",target:"_blank",rel:"noopener noreferrer"},g={href:"https://tiewei.github.io/posts/kubebuilder-vs-operator-sdk",target:"_blank",rel:"noopener noreferrer"},y={href:"https://github.com/fabric8io/kubernetes-client",target:"_blank",rel:"noopener noreferrer"};function h(f,n){const e=p("ExternalLinkIcon");return c(),o("div",null,[n[23]||(n[23]=s("h1",{id:"_9-kubernetes-应用封装与扩展",tabindex:"-1"},[s("a",{class:"header-anchor",href:"#_9-kubernetes-应用封装与扩展","aria-hidden":"true"},"#"),a(" 9. Kubernetes 应用封装与扩展")],-1)),n[24]||(n[24]=s("h3",{id:"_1-kustomize",tabindex:"-1"},[s("a",{class:"header-anchor",href:"#_1-kustomize","aria-hidden":"true"},"#"),a(" 1. Kustomize")],-1)),n[25]||(n[25]=s("p",null,"当我们需要在 Kubernetes 部署应用时，往往是编写许多 yaml 文件来部署各种资源对象，并且同一个应用针对不同的环境可能需要编写不同的 yaml 文件，这个过程往往非常繁琐。",-1)),s("p",null,[n[1]||(n[1]=a("为了解决这个问题 Kubernetes 推出了 ")),s("a",u,[n[0]||(n[0]=a("Kustomize")),i(e)]),n[2]||(n[2]=a(" 工具，官方称为 Kubernetes 原生配置管理工具。Kustomize 将我们应用部署所需要的信息分为不变的 base 配置和容易变化 overlay 配置，最终将文件合起来成为一个完整的定义文件。类似于 Docker 镜像分层的概念，最终所有的层次合并起来成为一个完整的应用镜像。"))]),n[26]||(n[26]=l(`<p>Kustomize 最常见的用途就是根据不同的环境生成不同的部署文件，在基准 base 文件的基础上，定义不同的 Overlay 文件，在通过 Kustomization 文件的定义进行整合。</p><ul><li>base 文件，基准 yaml 文件</li><li>overlay 文件，针对不同需求设置的 yaml 文件 Kustomization.yaml：整合 base 和 overlay 以生成完整的部署配置。</li></ul><p>kubectl 命令已经内置了 kustomize 命令，当我们定义好上述文件后，可以通过 kubectl kustomize overlays/dev 查看生成的部署内容，通过 <code>kubectl apply -k overlays/dev</code>直接执行部署。</p><p>下面是一个简单的示例，我们部署一个应用，在测试环境部署一个 1 个副本并且使用 test 镜像，在生产环境部署 5 个副本并使用最新的镜像。正常情况下我们需要定义两个完整的 yaml 文件分别去生产和测试环境部署，如果使用 Kustomize 可以想下面这样定义：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ tree app
app
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ol start="3"><li></li></ol><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>查看应用状态
...
\`-- values.yaml
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>指定名称执行命令后会自动生成同名目录以及相关文件：</p><ul><li>charts 目录: 存储依赖的 chart</li><li>Chart.yaml：存放 chart 的元信息以及一些 chart 空间。</li><li>template 目录：存放最终生成 Kubernetes 清单 yaml 的模板文件</li><li>template.test: 测试文件，不会被按安装到集群中，可以由 helm test 执行测试。</li><li>values.yaml：定义值，在 helm 渲染模板时传递给模板覆盖默认值。</li></ul><p>默认生成的 chart 是一个 Nginx 的应用，我们可以直接安装运行</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ helm <span class="token function">install</span> nginx-1-16  <span class="token builtin class-name">.</span>
NAME: nginx-1-16
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>可以看到该库包含了 first-chart 和 my-app 两个 chart，my-app 有两个版本分别是 0.1.0 和 0.2.0，并且包含了各个版本的 url 下载路径。</p>`,32)),s("p",null,[n[12]||(n[12]=a("我们可以使用 ")),s("a",k,[n[11]||(n[11]=a("ChartMuseum")),i(e)]),n[13]||(n[13]=a("、Google 云端存储或者自己搭建静态 web 服务器来实现存储库，如果 Chart 可以公开的话 Github Pages 也是一个非常好的选择。下面以 Github Pages 为例看下如何使用存储库："))]),n[30]||(n[30]=l(`<ol><li>创建 Github repo 并设置 Pages</li></ol><p>首先我们在 Github 创建一个 public 仓库 <img src="https://i-blog.csdnimg.cn/blog_migrate/9946f2d8888386e62d8dab342e8cc41f.png" alt="在这里插入图片描述"></p><p>一个仓库创建完成后设置 Pages，选择 main 分支，表示每次 main 分支更新时都会重新部署 Github Pages 站点，如果有自定义的域名也可以设置域名。这里设置的域名是 https://zouyingjie.cn/naive-charts-repo/。</p><p><img src="https://i-blog.csdnimg.cn/blog_migrate/9aa32d354e49a1e67bfc09877c6315eb.png" alt="在这里插入图片描述"></p><ol start="2"><li>添加 chart 到存储库</li></ol><p>GIthub 仓库创建完成后，我们可以 clone 到本地然后添加 index.yaml 文件以及 Chart，将其转为真正的 chart 存储库。</p><ul><li>Clone 项目 并创建 Chart</li></ul><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ <span class="token function">git</span> clone https://github.com/zouyingjie/naive-charts-repo.git
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>仅仅定义了 CRD 以及自定义对象，还无法做到非常好的扩展。Kubernetes 的内置对象，比如 Deployment 之所以能执行滚动升级，是因为有对应的控制器在执行状态拟合。对于 CRD 也一样，我们的自定义对象也需要一个控制器来执行操作，这是由 Operator 提供的。</p><h4 id="_3-2-创建部署-operator" tabindex="-1"><a class="header-anchor" href="#_3-2-创建部署-operator" aria-hidden="true">#</a> 3.2 创建部署 Operator</h4><blockquote><p>Operator是使用自定义资源（CR，Custom Resource，是CRD的实例），管理应用及其组件的 自定义Kubernetes控制器。高级配置和设置由用户在CR中提供。Kubernetes Operator基于嵌 入在Operator逻辑中的最佳实践将高级指令转换为低级操作。Kubernetes Operator监视CR类型并采取特定于应用的操作，确保当前状态与该资源的理想状态相符。--- Red Hat</p></blockquote><p>简单来说：<code>Operator = CRD + Controller。</code></p><p>Kubernetes 本身提供的资源对象和控制器只能对最通用的操作做抽象，比如 CronJob 执行定时任务，Deployment 执行滚动升级等。但对于应用特定操作 Kubernetes 是做不到的，比如在 Kuberetes 部署一个 3 节点ElasticSearch 集群，需要将 StatefulSet、ConfigMap、Service 文件等悉数配置好才可以成功。之所以要详细配置是因为 Kubernetes 并不知道如何将 ElasticSearch 部署为一个集群，但如果 Kuberetes 本身有一个 ElasticSearch 的资源对象，并且有控制器基于该资源对象进行状态拟合，那我们可以很方便将部署 ElasticSearch 集群这些“高级指令”转化为 Kubernetes 可以执行的的 “低级操作”。</p><div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> elasticsearch.k8s.elastic.co/v1
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ul><li>controller 目录：这里写的就是具体的控制逻辑，我们编写 operator 时主要的工作量基本都是编写控制逻辑。这里的逻辑非常简单，每当有 MonkeyPod 创建，我们的 controller 会创建同名的 Pod 并打上 &quot;monkey&quot;: &quot;stupid-monkey&quot; 标签:</li></ul><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>
\`\`\`go
